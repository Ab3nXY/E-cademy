...
const FinalExam = () => {
  ...
  useEffect(() => {
    // Fetch final exam questions for the course
    axios.get(`/api/courses/${courseId}/final-exam/assessments/`)
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      axios.post(`/api/courses/${courseId}/final-exam/assessments/submit/`, values)
        .then(response => {
          console.log('Submitted answers:', values.answers);
          navigate.push(`/courses/${courseId}/final-exam/result`, { result: response.data });
        })
